...
it('returns fillable attributes correctly', function() {
    $result = $this->traitObject->getFillable();

    expect($result)->toBe(['name', 'email']);
});

it('returns guarded attributes correctly', function() {
    $result = $this->traitObject->getGuarded();

    expect($result)->toBe(['password']);
});

it('returns hidden attributes correctly', function() {
    $result = $this->traitObject->getHidden();

    expect($result)->toBe(['password']);
});

it('returns visible attributes correctly', function() {
    $result = $this->traitObject->getVisible();

    expect($result)->toBe(['name', 'email']);
});

it('sets model attribute using mutator', function() {
    $result = $this->traitObject->setModelAttribute('name', 'John');

    expect($result)->toBe($this->traitObject);
});

it('gets model attribute using mutator', function() {
    $result = $this->traitObject->getModelAttribute('name', 'John');

    expect($result)->toBe('John Doe');
});

it('determines if a get mutator exists for an attribute', function() {
